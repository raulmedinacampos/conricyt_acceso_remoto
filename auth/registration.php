...
<p>Reconozco que el uso de <strong>mi Clave de Acceso Remoto es personal, privada e intransferible y que el periodo de vigencia es de un año siempre que mi institución continúe siendo parte del Consorcio</strong>, por lo que en ningún caso podré prestarla a ningún otro usuario, incluyendo cualquier otro miembro de la comunidad académica a la que pertenezco.</p>
...
if(isset($_GET['profile'])){
	...
	if($_POST['setunique'] == $_SESSION['checkUnique'] && isset($_POST['Registrar']) && $_POST['honeypot'] == '') {
		...
		$user = new User(
			...
			($_POST['institution'] == 475) ? (($_POST['perfil_imss']) ? (int)$_POST['perfil_imss'] : 19) : (int)$_POST['profile'],
		...
		// Se generan las credenciales en el momento que se hace el registro		
		...
		if((bool)$profile) {
			if($_GET['profile'] == "sni") {
				$institutions = $pdo->query("SELECT 509 AS id, 'Sistema Nacional de Investigadores' AS inst_name")->fetchAll();
			} else {
